...
Refresh.Web.DefaultColorPickerSettings = {
	...
	clientFilesPath: 'refresh_web/colorpicker/'
};
...
Refresh.Web.ColorPicker.prototype = {
	...
	setColorMode: function(colorMode) {
		...
		switch (colorMode) {
			...
			case 's':
				...
				console.log("_saturationRadio = "+this._saturationRadio.checked);
				// MAP
				...
				// SLIDER
				// bottom: color
				console.log("_barL3 = "+this.color.hex);
			...
			case 'v':
				...
				console.log("_valueRadio.checked = "+this._valueRadio.checked);
				// MAP
				// bottom: nothing
	...
	mapValueChanged: function() {
		...
		switch(this.ColorMode) {
			...
			case 's':
			//console.log("this._cvp._hueInput.value = "+this._map.xValue);
			
				this._cvp._hueInput.value = this._map.xValue;
				this._cvp._valueInput.value = 100 - this._map.yValue;
				console.log("x = "+this._map.xValue);//x
				console.log("y = "+this._map.yValue);// y
				if(debug_map == true){
					console.log("this._cvp._hueInput.value = "+this._cvp._hueInput.value);//x
					console.log("_cvp._valueInput.value = "+this._cvp._valueInput.value);// y
				}
				break;
	...
	sliderValueChanged: function() {
		console.log("sliderValueChanged() ");//center bar map
		switch(this.ColorMode) {
			case 'h':
	...
	console.log("positionMapAndSliderArrows() ");//only first time, input value from inputbox
		this.color = this._cvp.color;
		...
		switch(this.ColorMode) {
			...
			case 's':
				...
				console.log("sliderValue = "+sliderValue);//only first time .ALL 0 and 0
				break;
		...
		this._slider.setArrowPositionFromValues();//jimmy:set 

		// color map
		var mapXValue = 0;
		var mapYValue = 0;
		switch(this.ColorMode) {//input value from inputbox
			case 'h':
		...
		console.log("_map.xValue = "+this._map.xValue);//
		console.log("_map.yValue = "+this._map.yValue);//
		this._map.setArrowPositionFromValues();
	},
	updateVisuals: function() {//jimmy: run every time when user drag arrow
		console.log("updateVisuals()");
		this.updatePreview();//right map
		this.updateMapVisuals();
		this.updateSliderVisuals();//center slider
	},
	updatePreview: function() {
		//console.log("updatePreview() ");//
		try {
			this._preview.style.backgroundColor = '#' + this._cvp.color.hex;//set cp1_Preview color
		} catch (e) {}
	},
	updateMapVisuals: function() {
		//console.log("updateMapVisuals() ");//for what?
		this.color = this._cvp.color;
	...
	updateSliderVisuals: function() {
		console.log("updateSliderVisuals() ");//update cp1_ColorBar
		this.color = this._cvp.color;
	...
	setAlpha: function(obj, alpha) {
		console.log("setAlpha()");//set left map hue
		if (this.isLessThanIE7) {
			//console.log("isLessThanIE7()");
			var src = obj.pngSrc;
